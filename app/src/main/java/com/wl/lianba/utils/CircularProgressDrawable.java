 ...
 * <p/>
 * This drawable will produce a circular shape with a ring surrounding it. The
 * ring can appear both filled and give a little cue when it is empty.
 * <p/>
 * The inner circle size, the progress of the outer ring and if it is loading
 * parameters can be controlled, as well the different colors for the three
 * components.
 *
...
public class CircularProgressDrawable extends Drawable {
     ...
     * <p/>
     * The inner ring is supposed to defaults stay 3/4 radius off the outer ring
     * at (75% scale), but this property can make it grow or shrink via this
     * equation: OuterRadius * Scale.
     * <p/>
     * A 100% scale will make the inner circle to be the same radius as the
     * outer ring.
     */
     ...
     * <p/>
     * The progress of the circle. If {@link #setIndeterminate(boolean)
     * indeterminate flag} is set to FALSE, this property will be used to
     * indicate the completion of the outer circle [0..1f].
     * <p/>
     * If set to TRUE, the drawable will activate the loading mode, where the
     * drawable will show a 90º arc which will be spinning around the outer
     * circle as much as progress goes.
     */
    ...
    /**
     * Scale of the inner circle. It will affect the inner circle size on this
     * equation: ([Biggest length of the Drawable] / 2) - (ringWidth / 2) *
     * scale.
     */
     ...
     * @param circleScale Scale difference between the outer ring and the inner
     *            circle
     ...
     */
    CircularProgressDrawable(int ringWidth, float circleScale, int outlineColor, int ringColor,
                             int centerColor, int width, int height, int padding) {
        this.progress = 0;
        this.outlineColor = outlineColor;
        this.ringColor = ringColor;
        this.centerColor = centerColor;
    ...
    public void draw(Canvas canvas) {
        ...
        if (width != 0 && height != 0) {
            ...
            canvas.drawCircle(bounds.centerX(), bounds.centerY(), width / 2, paint);

            int w = width - padding;
            int h = height - padding;

            int left = bounds.left + (bounds.right - bounds.left - w) / 2;
            int right = left + w;
            int top = bounds.top + (bounds.bottom - bounds.top - h) / 2;
            int bottom = top + h;
        ...
        float offsetX = (bounds.width() - outerRadius * 2) / 2 + bounds.left;
        float offsetY = (bounds.height() - outerRadius * 2) / 2 + bounds.top;
        ...
        canvas.drawCircle(bounds.centerX(), bounds.centerY(), outerRadius - ringWidth / 2, paint);
        ...
        if (indeterminate) {
            canvas.drawArc(arcElements, -progress, 0, false, paint);
        } else {
            canvas.drawArc(arcElements, 0, -progress, false, paint);
        }
    }

    @Override
    protected boolean onLevelChange(int level) {
        setProgress(level * 1.0f / 10000f);
        return true;
    }
     ...
     * <p/>
     * Will output a correct value only when the indeterminate mode is set to
     * FALSE.
     *
     ...
     *
     * @return TRUE if the Drawable is in indeterminate mode or FALSE if it is
     *         in progress mode.
     */
     ...
     * <p/>
     * The indeterminate parameter will change the behavior of the Drawable. If
     * the indeterminate mode is set to FALSE, the outer ring will be able to be
     * filled by using {@link #setProgress(float) setProgress}.
     * <p/>
     * Otherwise the drawable will enter "loading mode" and a 90º arc will be
     * able to be spinned around the inner circle.
     * <p/>
     * <b>By default, indeterminate mode is set to FALSE.</b>
     *
     * @param indeterminate TRUE to activate loading mode. FALSE to activate
     *            progress mode.
     */
    ...
    public static class Builder {
        ...
        public CircularProgressDrawable create() {
            return new CircularProgressDrawable(ringWidth, circleScale, outlineColor, ringColor,
                    centerColor, width, height, padding);
        }
