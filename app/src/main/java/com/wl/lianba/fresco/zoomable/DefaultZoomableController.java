...
package com.wl.lianba.fresco.zoomable;
...
import com.wl.lianba.fresco.gestures.TransformGestureDetector;
...
public class DefaultZoomableController implements ZoomableController,
     ...
     * <p>
     * Note that the hierarchy performs scaling as well, which is not accounted
     * here, so the actual scale factor may differ.
     */
     ...
     * system, and writes the transformed points back into the array. Points are
     * represented by float array of [x0, y0, x1, y1, ...].
     *
    ...
    /**
     * Gets the zoomable transformation Internal matrix is exposed for
     * performance reasons and is not to be modified by the callers.
     */
    @Override
    ...
    /**
     * Sets the zoomable transformation. Cancels the current gesture if one is
     * happening.
     */
     ...
     * in the center.
     * <p>
     * It might not be possible to center imagePoint (= a corner for e.g.), in
     * those cases the view will be adjusted so that there are no black bars in
     * it. Resets any previous transform and cancels the current gesture if one
     * is happening.
     *
    ...
    public void zoomToImagePoint(float scale, PointF imagePoint) {
        ...
        mActiveTransform.postTranslate(mViewBounds.centerX() - points[0], mViewBounds.centerY()
                - points[1]);
        limitTranslation();
    }
     ...
     * smaller than view) centers the image.
     * 
    ...
    public boolean isOutside() {
        ...
        bounds.set(mImageBounds);
        mActiveTransform.mapRect(bounds);
        return (bounds.right <= mViewBounds.width()) || (bounds.left >= 0);
    }
